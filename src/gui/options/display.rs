// GUI/OPTIONS/DISPLAY
// DRAW A SETTINGS PAGE FOR DISPLAY BRIGHTNESS:
// – BRIGHTNESS ARC GAUGE (RED‑GREEN)
// – STATIC BRIGHTNESS ICON INSIDE THE ARC
// – GLOWING TOGGLE SWITCH AT THE BOTTOM (DISPLAY ON/OFF)
    ...
    critical_section::with(|_cs| unsafe {
            ...
            core::ptr::addr_of!(HIT_AREA).read().as_ref()
        {
            if crate::gui::hit_test(x, y, area) {
                // TOGGLE DISPLAY STATE (ON/OFF)
                let is_on: bool = crate::load!(crate::state::DISPLAY_STATE);
                crate::store!(crate::state::DISPLAY_STATE, !is_on);
...
// ───────────────────────────────────────────────────────────────────────
// HANDLE SWIPE
// SWIPE UP & DOWN TO CONTROL THE BRIGHTNESS
pub fn handle_swipe(
    ...
    match direction {
        ...
        | crate::components::ft3168::SwipeDirection::Down => {
            ...
            let brightness_change = -raw_delta; // POSITIVE FOR UP, NEGATIVE FOR DOWN

            // SENSITIVITY: 1% PER 2 PIXELS OF VERTICAL MOVEMENT
            let sensitivity = 2;
            let delta_bright = brightness_change / sensitivity;

            if delta_bright != 0 {
                let current: u8 = crate::load!(crate::state::DISPLAY_BRIGHTNESS);
                let new_val =
                    (current as i32 + delta_bright).clamp(0, 100) as u8;
                crate::store!(crate::state::DISPLAY_BRIGHTNESS, new_val);
                defmt::info!("Display brightness adjusted to {}%", new_val);
            }
        }
        _ => {}
    }
...
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    let display_on: bool = crate::load!(crate::state::DISPLAY_STATE);
    let brightness: u8 = crate::load!(crate::state::DISPLAY_BRIGHTNESS);
    ...
    let header_style = embedded_ttf::FontTextStyleBuilder::new(bold_font.clone())
        .font_size(80) // LONGER HEADER - SMALLER HEADER
    ...
    // BACKGROUND ARC (FULL GREY CIRCLE)
    ...
    // BRIGHTNESS ICON INSIDE THE ARC
    let icon_bytes = crate::base::assets::SETTINGS_BRIGHTNESS_PNG;
    ...
    let progress = if display_on { 1.0 } else { 0.0 };
    draw_toggle_switch(
        fb,
