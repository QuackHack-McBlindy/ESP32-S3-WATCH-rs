// GUI/OPTIONS/SPEAKER
// DRAW A SETTINGS PAGE FOR THE SPEAKER:
// – VOLUME ARC GAUGE (RED‑GREEN)
// – SPEAKER ICON INSIDE THE ARC (CHANGES WITH MUTE & VOLUME)
    ...
    critical_section::with(|_cs| unsafe {
            ...
            core::ptr::addr_of!(HIT_AREA).read().as_ref()
        {
            if crate::gui::hit_test(x, y, area) {
                // TOGGLE MUTE STATE
                let is_muted: bool = crate::load!(crate::state::SPEAKER_MUTED);
                crate::store!(crate::state::SPEAKER_MUTED, !is_muted);
...
// ───────────────────────────────────────────────────────────────────────
// HANDLE SWIPE
// CONTROL SPEAKER VOLUME BY SWIPING UP & DOWN
pub fn handle_swipe(
    ...
    match direction {
        ...
        | crate::components::ft3168::SwipeDirection::Down => {
            // SCREEN Y INCREASES DOWNWARD - UP SWIPE NEGATIVE DELTA
            ...
            if delta_vol != 0 {
                let current: u8 = crate::load!(crate::state::SPEAKER_VOLUME);
                let new_val =
                    (current as i32 + delta_vol).clamp(0, 100) as u8;
                crate::store!(crate::state::SPEAKER_VOLUME, new_val);
                crate::set_speaker_volume(new_val);
                defmt::info!("Speaker volume adjusted to {}%", new_val);
            }
        }
        _ => {}
    }
...
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    let is_muted: bool = crate::load!(crate::state::SPEAKER_MUTED);
    let volume: u8 = crate::load!(crate::state::SPEAKER_VOLUME);
    ...
    // SPEAKER ICON - IN ARC
    let icon_bytes = if is_muted || volume == 0 {
        crate::base::assets::SETTINGS_SPEAKER_OFF_PNG
    } else if volume <= 33 {
        crate::base::assets::SETTINGS_VOLUME_0_PNG
    } else if volume <= 66 {
        crate::base::assets::SETTINGS_VOLUME_1_PNG
    } else {
        crate::base::assets::SETTINGS_VOLUME_2_PNG
    };
