// GUI/OPTIONS/CPU
// DRAWS SETTINGS PAGE FOR CPU FREQUENCY – ARC GAUGE + CPU ICON + MHZ VALUE
// SWIPE UP/DOWN TO CYCLE BETWEEN 80, 160, 240 MHZ
...
/// DRAWS THE CPU FREQUENCY SETTINGS PAGE
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    ...
    // READ CURRENT CPU FREQUENCY (U16: 80, 160, OR 240)
    let freq: u16 = crate::load!(crate::state::CPU_FREQ);
    // MAP TO 0–100% FOR ARC FILL: 80 > 0%, 160 > 50%, 240 > 100%
    let percent = match freq {
        80 => 0u8,
        160 => 50,
        240 => 100,
        _ => 0, // FALLBACK
    ...
    // FILL ARC – GRADIENT FROM RED (80 MHZ) TO GREEN (240 MHZ)
    ...
    if let core::result::Result::Ok(icon_png) =
        embedded_png::Png::load_from_bytes(crate::base::assets::SETTINGS_CPU_PNG)
    {
    ...
    // TEXT: "XXX MHZ" BELOW THE ICON
    let text_font = rusttype::Font::try_from_bytes(crate::base::assets::ROBOTO_BOLD).unwrap();
    let text_style = embedded_ttf::FontTextStyleBuilder::new(text_font)
        .font_size(82)
        .text_color(crate::gui::colors::WHITE)
        .build();
    let text_align = embedded_graphics::text::TextStyleBuilder::new()
        .alignment(embedded_graphics::text::Alignment::Center)
        .build();
    let freq_text = format_freq(freq);
    let _ = embedded_graphics::Drawable::draw(
        &embedded_graphics::text::Text::with_text_style(
            &freq_text,
            embedded_graphics::prelude::Point::new(center_x, center_y + 90),
            text_style,
            text_align,
        ),
        fb,
...
/// FORMATS FREQUENCY AS "XXX MHZ"
fn format_freq(mhz: u16) -> heapless::String<16> {
    let mut s = heapless::String::new();
    core::fmt::Write::write_fmt(&mut s, format_args!("{} MHz", mhz)).ok();
    s
}

// ───────────────────────────────────────────────────────────────────────
// TOUCH HANDLING – NO TOGGLE ACTION, RETURNS NONE
pub fn handle_touch(_x: i32, _y: i32) -> Option<crate::gui::TouchAction> {
    None
}

// ───────────────────────────────────────────────────────────────────────
// HANDLE SWIPE – CYCLE THROUGH 80/160/240 MHZ
pub fn handle_swipe(
    ...
    match direction {
        ...
        | crate::components::ft3168::SwipeDirection::Down => {
            ...
            // INTERPRET SWIPE UP (POSITIVE VALUE_CHANGE) AS INCREASE
            let value_change = -raw_delta;

            // USE A THRESHOLD TO AVOID ACCIDENTAL CHANGES: ONLY CHANGE IF SWIPE DISTANCE > 10 PIXELS
            if value_change.abs() > 10 {
                let current: u16 = crate::load!(crate::state::CPU_FREQ);
                let new_freq = if value_change > 0 {
                    // SWIPE UP: NEXT HIGHER FREQUENCY
                    match current {
                        80 => 160,
                        160 => 240,
                        _ => 240, // ALREADY MAX
                    ...
                    // SWIPE DOWN: NEXT LOWER FREQUENCY
                    match current {
                        240 => 160,
                        160 => 80,
                        _ => 80, // ALREADY MIN
                ...
                if new_freq != current {
                    crate::components::frequency::set_cpu_mhz(new_freq);
                    crate::store!(crate::state::CPU_FREQ, new_freq);
                    defmt::info!("🖥️ FREQUENCY {} MHZ", new_freq);
                }
            }
        }
        _ => {}
