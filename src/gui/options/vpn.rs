// GUI/OPTIONS/VPN
// DRAW A SETTINGS PAGE FOR VPN ON/OFF
    ...
    critical_section::with(|_cs| unsafe {
            ...
            core::ptr::addr_of!(HIT_AREA).read().as_ref()
        {
            if crate::gui::hit_test(x, y, area) {
                let is_on = crate::load!(crate::state::WG_STATE);
                crate::store!(crate::state::WG_STATE, !is_on);
...
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    ...
    let icon_bytes = if is_on {
        crate::base::assets::SETTINGS_VPN_ON_PNG
    } else {
        crate::base::assets::SETTINGS_VPN_OFF_PNG
    };
        ...
        for sy in 0..img_h {
            for sx in 0..img_w {
                ...
                if let core::option::Option::Some(color) = icon_png.pixels()[idx] {
                    // Draw the pixel directly from the selected icon
    ...
    let area = crate::gui::HitArea {
        ...
        action: crate::gui::TouchAction::SettingsToggleVpn,
    };
    critical_section::with(|_cs| unsafe {
