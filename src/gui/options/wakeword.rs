// GUI/OPTIONS/WAKEWORD
// DRAW A SETTINGS PAGE FOR TOGGLE WAKE WORD DETECTION ON/OFF
// BIG COLOR CODED TOGGLE SWITCH AND A ICON
...
// ───────────────────────────────────────────────────────────────────────
// HANDLE TOUCH
// TOGGLES WAKE WORD ENABLE STATE
    ...
    critical_section::with(|_cs| unsafe {
            ...
            core::ptr::addr_of!(HIT_AREA).read().as_ref()
        {
            if crate::gui::hit_test(x, y, area) {
                let is_on = crate::load!(crate::state::WAKE_WORD_ENABLED);
                crate::store!(crate::state::WAKE_WORD_ENABLED, !is_on);
...
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    ...
    // STATIC WAKE WORD ICON
    let icon_bytes = crate::base::assets::SETTINGS_WAKE_WORD_PNG;
...
pub fn draw_toggle_switch(
    ...
    let _ = embedded_graphics::Drawable::draw(
        &embedded_graphics::primitives::Rectangle::new(
            embedded_graphics::prelude::Point::new(track_left - glow_expand, track_top - glow_expand),
            embedded_graphics_core::geometry::Size::new(
                (track_w + 2 * glow_expand) as u32,
                (track_h + 2 * glow_expand) as u32,
            ),
        )
        .into_styled(embedded_graphics::primitives::PrimitiveStyle::with_fill(glow_color)),
        fb,
    );
    ...
    let _ = embedded_graphics::Drawable::draw(
        &embedded_graphics::primitives::Rectangle::new(
            ...
            embedded_graphics_core::geometry::Size::new(track_w as u32, track_h as u32),
        )
        .into_styled(embedded_graphics::primitives::PrimitiveStyle::with_fill(track_fill)),
        fb,
    );
    ...
    let _ = embedded_graphics::Drawable::draw(
        ...
        .into_styled(embedded_graphics::primitives::PrimitiveStyle::with_fill(dark_gray)),
        fb,
    );

    let _ = embedded_graphics::Drawable::draw(
        &embedded_graphics::primitives::Circle::new(
            embedded_graphics::prelude::Point::new(thumb_center_x - thumb_radius, thumb_center_y - thumb_radius),
            thumb_diameter,
        )
        .into_styled(embedded_graphics::primitives::PrimitiveStyle::with_fill(white)),
        fb,
    );
}
