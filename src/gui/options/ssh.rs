// GUI/OPTIONS/SSH
// DRAW A SETTINGS PAGE FOR TOGGLE SSH ON/OFF
    ...
    critical_section::with(|_cs| unsafe {
            ...
            core::ptr::addr_of!(HIT_AREA).read().as_ref()
        {
            if crate::gui::hit_test(x, y, area) {
                // TOGGLE SSH STATE (ON/OFF)
                let is_on: bool = crate::load!(crate::state::SSH_STATE);
                defmt::info!("SSH state: {}", !is_on);
                crate::store!(crate::state::SSH_STATE, !is_on);
...
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    ...
    let icon_bytes = crate::base::assets::SETTINGS_TERMINAL_PNG;
