// GUI/OPTIONS/API
// DRAW A SETTINGS PAGE FOR TOGGLE API ON/OFF
    ...
    critical_section::with(|_cs| unsafe {
            ...
            core::ptr::addr_of!(HIT_AREA).read().as_ref()
        {
            if crate::gui::hit_test(x, y, area) {
                // TOGGLE API STATE (ON/OFF)
                let is_on: bool = crate::load!(crate::state::API_STATE);
                crate::store!(crate::state::API_STATE, !is_on);
...
pub fn draw(fb: &mut crate::components::framebuffer::Framebuffer) {
    ...
    // STATIC API ICON
    let icon_bytes = crate::base::assets::SETTINGS_API_PNG;
